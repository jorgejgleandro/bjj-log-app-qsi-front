...
const getList = async (route_name, myTable) => {
  ...
  fetch(url, {
    ...
    .then((data) => {data[route_name].forEach(item => {insertList(item, myTable, route_name)})})
    .catch((error) => {
      console.error('Error:', error);
    });
}
...
const getItemObj = async (route_name, ...args) => {
  if (route_name === 'tecnica') {
    ...
    return ({
      ...
      'trtbps': document.getElementById(args[4]).value,
      'chol': document.getElementById(args[5]).value,
      'fbs': document.getElementById(args[6]).value,
      'restecg': document.getElementById(args[7]).value,
      'thalachh': document.getElementById(args[8]).value,
      'exng': document.getElementById(args[9]).value,
      'oldpeak': document.getElementById(args[10]).value,
      'slp': document.getElementById(args[11]).value,
      'caa': document.getElementById(args[12]).value,
      'thall': document.getElementById(args[13]).value
    });    
  } else {
